...
$objControlSeccion = new ControlEntidad('seccion');
$arregloSecciones = $objControlSeccion->listar();
?>
                ...
                <div class="table-wrapper">
                    ...
                    <table class="table table-striped table-hover">
                        ...
                        <tbody>
                            ...
                            $total_registros = count($arregloSecciones);
                            ...
                            for($i = $inicio; $i < $fin && $i < $total_registros; $i++){
                                $num_registro = $i + 1;
                                $getid = $arregloSecciones[$i]->__get('id');
                                $getnombre = $arregloSecciones[$i]->__get('nombre');
                            ?>
                    ...
                    <div class="clearfix">
                        <div class="hint-text">Mostrando <b><?= $registros_mostrados ?></b> de <b><?= $total_registros ?></b> secciones</div>
                        <ul class="pagination">
                            <?php 
                            // Botón "Anterior"
                            echo "<li class='page-item " . ($pagina_actual == 1 ? 'disabled' : '') . "' style='" . ($pagina_actual == 1 ? 'display: none;' : '') . "'><a href='vistaSeccion.php?pagina=" . ($pagina_actual - 1) . "' class='page-link'>Anterior</a></li>";

                            // Números de página
                            for ($i=1; $i <= $total_paginas; $i++) { 
                                if($pagina_actual == $i){
                                    echo "<li class='page-item active'><a href='vistaSeccion.php?pagina=$i' class='page-link'>$i</a></li>";
                                }else{
                                    echo "<li class='page-item'><a href='vistaSeccion.php?pagina=$i' class='page-link'>$i</a></li>";
                                }
                            }
                            // Botón "Siguiente"
